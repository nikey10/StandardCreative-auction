  ...
  async setProvider(type: number) {
    ...
    switch (type) {
      ...
      case WalletTypes.mewWallet:
        const ETH_JSONRPC_URL = 'wss://ropsten.infura.io/ws/v3/287b5d14c20f4b7d9411d165fac6a688'
        const CHAIN_ID = 1
  ...
  login = async (type: number) => {
    ...
    switch (type) {
      case WalletTypes.metamask:
        ...
        //@ts-ignore
        this.web3.currentProvider.on('accountsChanged', async (accounts: Array<string>) => {
      ...
      case WalletTypes.walletConnect:
        ...
        if (accounts.length) {
          const address = accounts[0]
          store.dispatch(login({ address }, this.web3.utils.toHex(chainIdWalletConnect.toString())))
        } else {
          throw new Error('No account found')
        }
