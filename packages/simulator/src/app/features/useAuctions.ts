...
type AuctionState = {
  ...
  createBid: (tokenAddress: string, bidType: BidTypeEnum, ethAmount: number) => void
...
const useAuctions = create<AuctionState>(
  immer((set) => ({
    ...
    // createBid()
    createBid: (tokenAddress: string, bidType: BidTypeEnum, ethAmount: number) => {
      set((state) => {
        const auctionIndex = state.auctions.findIndex((auction) => auction.tokenAddress == tokenAddress)
        assert(auctionIndex !== -1, 'auction not found')
        const auction = state.auctions[auctionIndex]
        assert(auction.auctionEndTime > useClockTime.getState().time, 'auction ended')
        assert(
          useWallets.getState().wallets[useWallets.getState().activeWalletIndex].balance >= ethAmount,
          'not enough ETH'
        )
        ...
        if (bidType == BidTypeEnum['1of1']) {
          assert(
            ethAmount >=
              auction.top1of1BidAmount + (auction.minimumBidIncrementFor1of1 * auction.top1of1BidAmount) / 100,
          ...
          assert(ethAmount > auction.fixedPriceForOpenEdition, 'bid needs to be higher than open edition price')
          assert(
        ...
        useWallets.getState().decreaseWalletBalance(useWallets.getState().activeWalletIndex, ethAmount)
        const activeWallet = useWallets.getState().wallets[useWallets.getState().activeWalletIndex]
        auction.balance += ethAmount
        auction.logs.push({
          text: `${activeWallet.address.slice(0, 10)} bid ${ethAmount} ETH for the ${
        ...
        if (bidType == BidTypeEnum['1of1']) {
          ...
          if (!auction.openEditionBids[activeWallet.address]) auction.openEditionBids[activeWallet.address] = 0
    ...
    switchTop1of1BidToOpenEdition: (tokenAddress: string) => {
      set((state) => {
        ...
        if (!auction.openEditionBids[activeWallet.address]) auction.openEditionBids[activeWallet.address] = 0
        const amountToRefund = auction.top1of1BidAmount % auction.fixedPriceForOpenEdition
        const amountToTransfer = auction.top1of1BidAmount - amountToRefund
        useWallets.getState().increaseWalletBalance(useWallets.getState().activeWalletIndex, amountToRefund)
        auction.balance -= amountToRefund

        auction.logs.push({
          text: `${activeWallet.address.slice(0, 10)} was refunded ${amountToRefund} ETH`,
          createdAt: useClockTime.getState().time
        })
    ...
    finishAuction: (tokenAddress: string) => {
      set((state) => {
        ...
        assert(auctionIndex !== -1, 'auction not found')
        const auction = state.auctions[auctionIndex]
        assert(useClockTime.getState().time > auction.auctionEndTime, 'auction still in progress')
        ...
        if (winner == BidTypeEnum['1of1']) {
          ...
          for (const address of Object.keys(auction.openEditionBids)) {
            ...
            const walletIndex = useWallets.getState().wallets.findIndex((wallet) => wallet.address == address)
            if (walletIndex == -1) continue
            useWallets.getState().increaseWalletBalance(walletIndex, amount)
            auction.balance -= amount
            auction.logs.push({
              text: `${address?.slice(0, 10)} was refunded ${amount} ETH for losing OpenEdition bids`,
              createdAt: useClockTime.getState().time
            })
          }
        } else if (winner == BidTypeEnum.OpenEdition) {
          auction.nftsMinted += auction.totalOpenEditionBidsAmount / auction.fixedPriceForOpenEdition
          auction.finished = true
          _refundPreviousTop1of1Bidder(auction)
        }
        return state
...
const _checkWinner = (auction: AuctionType): BidTypeEnum | undefined => {
  if (auction.top1of1BidAmount > auction.totalOpenEditionBidsAmount) return BidTypeEnum['1of1']
  else if (auction.totalOpenEditionBidsAmount > 0) return BidTypeEnum.OpenEdition
  else return
}

const _refundPreviousTop1of1Bidder = (auction: AuctionType) => {
  const walletIndex = useWallets.getState().wallets.findIndex((wallet) => wallet.address == auction.top1of1BidAddress)
  if (walletIndex == -1) return

  useWallets.getState().increaseWalletBalance(walletIndex, auction.top1of1BidAmount)
