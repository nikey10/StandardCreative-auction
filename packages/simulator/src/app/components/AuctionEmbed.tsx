import useAuctions, { AuctionType, BidTypeEnum } from '@app/features/useAuctions'
...
const AuctionEmbed = ({ auction }: { auction: AuctionType }) => {
  ...
  const activeWallet = useWallets((state) => state.wallets[state.activeWalletIndex])
  const submitBid = () => {
    setError((error) => ({ ...error, submitBid: undefined }))
    try {
      useAuctions.getState().createBid(auction.tokenAddress, bidType, bidAmount)
    } catch (error) {
  ...
  const switchBidTypeTx = () => {
    ...
    try {
      useAuctions.getState().switchTop1of1BidToOpenEdition(auction.tokenAddress)
    } catch (error) {
  ...
  return (
    <div className="auction">
      <div>
        <div className="square" style={{ backgroundColor: '#' + auction.tokenAddress.slice(-6) }} />
        ...
        <br />
        <br />
        Minimum Bid Increment for 1-of-1: {auction.minimumBidIncrementFor1of1} ETH
        <br />
      ...
      <div className="section">
        ...
        <div className="function">
          ...
          <div className="bid-type">
            Bid type:{' '}
            <button disabled={bidType == BidTypeEnum['1of1']} onClick={() => setBidType(BidTypeEnum['1of1'])}>
              1OF1
            </button>{' '}
            <button disabled={bidType == BidTypeEnum.OpenEdition} onClick={() => setBidType(BidTypeEnum.OpenEdition)}>
              OPENEDITION
            </button>
          </div>
          <br />
          <div className="submit-bid">
            <button onClick={submitBid}>Submit as {activeWallet.address.slice(0, 10)}</button>{' '}
        ...
        <div className="function">
          ...
          <div className="submit-bid">
            <button onClick={switchBidTypeTx}>Submit as {activeWallet.address.slice(0, 10)}</button>{' '}
            {error.switchBidType && <div className="error">{error.switchBidType}</div>}
          </div>
        </div>
        <div className="function">
          <div className="function-name">finishAuction()</div>
          <div className="submit-bid">
            <button onClick={finishAuction}>Submit as {activeWallet.address.slice(0, 10)}</button>{' '}
            {error.finishAuction && <div className="error">{error.finishAuction}</div>}
          </div>
        </div>
        <div className="function">
          <button
            onClick={() => {
              ...
              download(JSON.stringify(auction, null, 4), `debug-${auction.tokenAddress}.json`, 'text/plain')
            }}
          >
            Save JSON
          </button>{' '}
          <button
            onClick={() => {
              useAuctions.setState((state) => {
                state.auctions = state.auctions.filter((_auction) => _auction.tokenAddress != auction.tokenAddress)
                return state
              })
            }}
          >
