    ...
    <div className="index">
      <div className="sidebar">
        ...
        <div className="clock">
          ...
          <button onClick={clockRunning ? useClockTime.getState().stopClock : useClockTime.getState().startClock}>
          ...
          <table>
            <tbody>
              {wallets.map((wallet, i) => (
                <tr key={wallet.address} className={`wallet ${activeWallet == i ? 'active' : ''}`}>
                  <td>{i + 1}</td>
                  <td>{wallet.address.slice(0, 10)}</td>
                  <td />
                  <td>
                    {wallet.balance} ETH <button onClick={() => updateWalletBalance(i)}>Change</button>
                  </td>
                  <td>
                    {activeWallet == i ? (
                      <button disabled>Active</button>
                    ) : (
                      <button onClick={() => useWallets.getState().setActiveWallet(i)}>Switch</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <button onClick={useWallets.getState().createWallet}>Generate New Wallet</button>
      </div>
      <div className="main">
