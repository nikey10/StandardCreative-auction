...
const CreateNFTContract = () => {
  ...
  const createNFTContract = async () => {
    ...
    if (deploying) return
    setDeploying(true)
    const deployTx = await new StandardCreativeNFT__factory(provider.getSigner()).deploy(
      name,
      symbol,
  ...
  return (
    <div className="create-nft-contract">
      ...
      <input type="text" placeholder="Symbol ($ABCD)" value={symbol} onChange={(e) => setSymbol(e.target.value)} />
      <input
        type="text"
      ...
      <pre>{JSON.stringify({ deployTx, deployedContractAddress }, null, 2)}</pre>
      <style jsx>{`
        .create-nft-contract {
          padding: 10px;
