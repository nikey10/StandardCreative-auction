...
contract StandardCreativeAuction is IStandardCreativeAuction, Ownable, ReentrancyGuard {
  ...
  modifier auctionExists(uint256 id) {
    require(_auctions[id].nftContract != address(0), "Auction doesn't exist");
    _;
  }
  modifier auctionEnded(uint256 id) {
    require(_auctions[id].startTime != 0, 'Auction not started');
    require(block.timestamp > _auctions[id].startTime.add(_auctions[id].duration), 'Auction not finished');
    _;
  }
  ...
  event AuctionBid(uint256 indexed id, bool oneOfOne, uint256 amount, address indexed bidder, uint256 duration);
  event AuctionFinished(uint256 indexed id);
  ...
  function createAuction(
    ...
    require(IERC165(nftContract).supportsInterface(interfaceId), "Doesn't support NFT interface");
  ...
  // Create Bid
  function createBid(uint256 auctionId, bool oneOfOne) public payable auctionExists(auctionId) nonReentrant {
    // Lookup auction
    ...
    if (oneOfOne) {
      require(
        msg.value >= auction.topBidAmount.add(auction.topBidAmount.mul(auction.minBidIncrement).div(100)),
        'Bid is too low'
      );
      ...
      // Emit event
      emit AuctionBid(auctionId, true, msg.value, _msgSender(), auction.duration);
    } else {
      ...
      emit AuctionBid(auctionId, false, msg.value, _msgSender(), 0);
    }
  ...
  function _checkWinnerIsOneOfOne(uint256 auctionId) private view auctionExists(auctionId) returns (bool) {
    return _auctions[auctionId].totalOpenEditionBids > _auctions[auctionId].topBidAmount ? false : true;
  }
}
